...
// I achieved the hamburger menu with cancel toggle by combining React’s useState and Tailwind’s responsive classes:
//  first, I used a state variable isOpen to track whether the menu is open or closed, then I conditionally rendered 
//  the button so it shows the hamburger icon (☰) when closed and the cancel icon (✖) when open; for responsiveness, 
//  I used Tailwind’s hidden md:flex to display the full navbar only on larger screens and md:hidden to show the toggle
//   button only on smaller screens, while the mobile menu itself is conditionally rendered with {isOpen && (...)} so it
//    only appears when the state is true, and I added onClick={() => setIsOpen(false)} to each link so the menu closes 
//    when a link is clicked—together, this makes the navbar adapt to screen size and toggle seamlessly between open and 
//    closed states.
